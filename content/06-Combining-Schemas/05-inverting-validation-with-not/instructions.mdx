...
`not` keyword is used to invert the validation. If the instance is **not** valid against the subschema, the document is considered valid.
...
Modify the schema given to you in the side editor to ensure that the document is valid if `status` is not `null`.
