...
```
Now, modify the `phones` property of the given [schema](https://json-schema.org/learn/glossary#schema) on the <SideEditorLink/>, and specify minimum items as `1` and maximum items as `3`. Additionally constrain the phone numbers to be in the format `xxx-xxx-xxxx`.
