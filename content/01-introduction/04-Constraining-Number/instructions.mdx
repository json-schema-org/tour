...
Let's add dateOfBirth property field to the data. 
We want to apply the following constrains on the data: 
- `age` should be between 18 and 60
- `dateOfBirth.year` should be between 1954 and 2024 
...
now try to add the `age` and `dateOfBirth` properties to the schema given on the right side editor with the constraints mentioned above. 

You can also use the `exclusiveMinimum` and `exclusiveMaximum` keywords in the schema. 
For example: to set the age between 18 to 16 you can set `exclusiveMinimum : 15` and `exclusiveMaximum : 61`
