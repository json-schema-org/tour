...
JSON Schema is a language used for defining the _structure_  and _constraints_ of a JSON document.  A JSON Schema is a JSON document that you can use to validate or document another JSON, which we call a JSON instance.

You can use a _schema_ to validate or document an other JSON, which we call _instance_.
...
Now, try to Add an `age` property to the *schema* and, using the type keyword and set it `type` to `integer`. 
