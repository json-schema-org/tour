...
> **Hint:** Use the `patternProperties` keyword to define the pattern for the property name and `pattern` keyword to define the pattern for the value. you can set `pattern` to `^[A-Z]+$` to match all capital letters.
