...
keywords: "minimum, maximum, exclusiveMinimum, exclusiveMaximum, range"
---
...
now try to add the `age` and `salary` properties in the schema given on the right side editor with the constraints mentioned below using `exclusiveMinimum` and `exclusiveMaximum` keywords:
